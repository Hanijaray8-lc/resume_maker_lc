...
const Resume20 = ({
  ...
  // Data states - Resume1.js-ൽ இருந்து எடுக்கப்பட்ட தரவுகள்
  ...
  // Load data from localStorage - Resume1.js-ൽ இருந்து எடுக்கப்பட்ட தரவுகள்
  useEffect(() => {
  ...
  const renderRightSectionContent = (sectionId, inSidebar = false) => {
    ...
    const SectionHeader = inSidebar ? SidebarSectionHeader : MainSectionHeader;
      ...
      case 'profile':
        return (
          <Box key="profile" sx={{ mb: sectionSpacing / 10, ...textStyles }}>
            <SectionHeader 
              color={inSidebar ? palette.sidebarText : palette.accentColor} 
              font={font}
              fontSize={getHeadingSize()}
            >
              PROFILE
